...
public class CreateReportsScreenController implements Initializable {
	...
	/**
	 * Event handler triggered when the user selects a park from the park selection ComboBox.
	 * Updates the selectedPark field with the newly selected park.
	...
	/**
	 * Event handler triggered when the user selects a year from the year selection ComboBox.
	 * Updates the selectedYear field with the newly selected year.
	...
	/**
	 * Event handler triggered when the user selects a month from the month selection ComboBox.
	 * Updates the selectedMonth field with the newly selected month.
	...
	public void onGenerateReportClicked() {
		...
		if (selectedReportType == null || selectedMonth.equals("") || selectedYear.equals("")
				|| selectedPark == ParkNameEnum.None) {
			// TODO: Alert
			...
			if (selectedReportType == ReportType.CancellationsReport) {
				...
				report = new VisitsReport(Integer.parseInt(selectedMonth), Integer.parseInt(selectedYear),
						selectedPark);
