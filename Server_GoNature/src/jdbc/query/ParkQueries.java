...
//	}
	...
	public DatabaseResponse updateCurrentInParkValue(Order order, boolean direction) {
		try {
			...
			if (direction) {
				currentInParkUpdated += order.getNumberOfVisitors();
				orderQueries.updateOrderStatus(order, OrderStatusEnum.In_Park); // update the order to be in park
			} else {
				currentInParkUpdated -= order.getNumberOfVisitors();
				orderQueries.updateOrderStatus(order, OrderStatusEnum.Completed); // update the order to be completed
			}
			selectStmt.close();
			rs.close();
			PreparedStatement updateStmt = con.prepareStatement("UPDATE parks SET CurrentInPark = ? WHERE ParkId = ?;");
